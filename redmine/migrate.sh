...
# Backup
  ...
  mysqldump --user=${USERNAME} --password=${PASSWORD} --skip-extended-insert $DATABASE | gzip -c > redmine.sql.gz
  #echo "mysqldump --user=${USERNAME} --password=${PASSWORD} --skip-extended-insert $DATABASE > redmine.sql"
  #cp -f ${FILES}/* .
  #git add *
  #git commit -m "$MSG" 
  #git push --all origin
